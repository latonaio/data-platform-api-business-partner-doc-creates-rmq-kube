...
import (
	"context"
	"crypto/sha256"
	dpfm_api_input_reader "data-platform-api-business-partner-doc-creates-rmq-kube/DPFM_API_Input_Formatter"
	dpfm_api_output_formatter "data-platform-api-business-partner-doc-creates-rmq-kube/DPFM_API_Output_Formatter"
	"data-platform-api-business-partner-doc-creates-rmq-kube/config"
	"encoding/base64"
	"encoding/hex"
	"fmt"
...
func (c *DPFMAPICaller) AsyncDocCreates(
	...
	var fileExtension = input.BusinessPartner.GeneralDoc.FileExtension
	var docData = input.DocData

	var combinedString = fmt.Sprintf(
		"%v%v",
		time.Now().Unix(),
		input.BusinessPartner.GeneralDoc.DocVersionID,
	)
	...
	response := c.createSqlProcess(input, output, &dpfm_api_output_formatter.GeneralDoc{
		...
		DocVersionID:             input.BusinessPartner.GeneralDoc.DocVersionID,
		DocID:                    &dockId,
		FileExtension:            input.BusinessPartner.GeneralDoc.FileExtension,
		FileName:                 input.BusinessPartner.GeneralDoc.FileName,
		FilePath:                 directoryPath,
		DocIssuerBusinessPartner: input.BusinessPartner.GeneralDoc.DocIssuerBusinessPartner,
	}, errs, log)
